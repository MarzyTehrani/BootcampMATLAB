...
% or ones(size(xClean))
modelCoeff = A\b %Solve the system.
%Get the fitted curve:
pulseModel = A*modelCoeff;
%Plot:
hold on 
...
%Problem: can we estimate weight (y) using height and waist (x1 andx2)?
...
b = yClean;
A = [ones(size(x1Clean)), x1Clean, x1Clean.^2, ...
     x2Clean, x2Clean.^2, x1Clean.*x2Clean];
...
hold on
surf(X1, X2, modelOnGrid, 'EdgeAlpha', 0.9)
